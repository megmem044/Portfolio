...
export function createApp(options: AppOptions = {}) {
  ...
  app.use(cors({ origin: frontendOrigins, exposedHeaders: ['X-Correlation-Id', 'ETag', 'Cache-Control'] }));
  ...
  async function springRequest(path: string, request: Request, response: Response, body?: unknown) {
    ...
    if (ifNoneMatch && request.method === 'GET') headers.set('If-None-Match', ifNoneMatch);
  ...
  async function forward(response: Response, upstream: globalThis.Response) {
    for (const name of ['content-type', 'etag', 'cache-control', 'vary']) {
      const value = upstream.headers.get(name);
      if (value) response.setHeader(name, value);
    }
    if (upstream.status === 304) return response.status(304).end();
